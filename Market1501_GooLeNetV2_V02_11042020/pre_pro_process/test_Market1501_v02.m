...
Hist_query = forward_ori_center_crop_v03(net, queryImagesCell, param.test_batch_size, ...
...
Hist_test = forward_ori_center_crop_v03(net, testImagesCell, param.test_batch_size, ...
                                            param.crop_padding, param.force_square_size,  param.cropped_size, blob_name);
...
for n = 1 : length(multiQueryImagesCell)
    features_multiQueries = forward_ori_center_crop_v03(net, multiQueryImagesCell{n}, param.test_batch_size, ...
                                            param.crop_padding, param.force_square_size,  param.cropped_size, blob_name);
...
% L2 normalization on Hist_test & Hist_query
sum_val = sqrt(sum(Hist_test.^2));
sum_val = repmat(sum_val, [size(Hist_test,1), 1]);
sum_val2 = sqrt(sum(Hist_query.^2));
sum_val2 = repmat(sum_val2, [size(Hist_query,1), 1]);
Hist_test = Hist_test./sum_val;
Hist_query = Hist_query./sum_val2;
...
CMC = mean(CMC);
%% print result
fprintf('single query: mAP = %f, r1 precision = %f\r ',...
         mean(ap), CMC(1));
fprintf('multiple queries with avg pooling: mAP = %f, r1 precision = %f\r ',...
         mean(ap_avg), CMC_avg(1));
fprintf('multiple queries with max pooling: mAP = %f, r1 precision = %f\r ',...
...
fprintf(fin, 'iter: %d, \n',...
    iter);
fprintf(fin, '    single query: mAP = %f, r1 precision = %f\n ',...
         mean(ap), CMC(1));
fprintf(fin, '    multiple queries with avg pooling: mAP = %f, r1 precision = %f\n ',...
         mean(ap_avg), CMC_avg(1));
fprintf(fin, '    multiple queries with max pooling: mAP = %f, r1 precision = %f\n ',...
         mean(ap_max), CMC_max(1));
