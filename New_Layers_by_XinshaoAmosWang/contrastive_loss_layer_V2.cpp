...
void ContrastiveLossV2Layer<Dtype>::Forward_cpu(
  ...
  for (int i = 0; i < bottom[0]->num(); ++i) {
    ...
    if (static_cast<int>(bottom[2]->cpu_data()[i])) {  // similar pairs
      ...
      Dtype dist = std::max<Dtype>(sqrt(dist_sq_.cpu_data()[i]) - (margin - pn_margin),
          Dtype(0.0));
    ...
    if (propagate_down[i]) {
      ...
      for (int j = 0; j < num; ++j) {
        ...
        if (static_cast<int>(bottom[2]->cpu_data()[j])) {  // similar pairs
          ...
          Dtype mdist = sqrt(dist_sq_.cpu_data()[j]) - (margin - pn_margin);
          if (mdist > Dtype(0.0)) {
            caffe_cpu_axpby(
              channels,
              alpha,
          ...
          if (legacy_version) {
            ...
            mdist = margin - sqrt(dist_sq_.cpu_data()[j]);
            beta = -alpha;
          }
          if (mdist > Dtype(0.0)) {
            caffe_cpu_axpby(
