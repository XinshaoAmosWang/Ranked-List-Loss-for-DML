...
void DistanceWeightedValidPairLossV12Layer<Dtype>::LayerSetUp(
  ...
  scale_p = this->layer_param_.softmax_t_p_param().scale();
  base_p = this->layer_param_.softmax_t_p_param().base();
  //The negative parameters of power_x_scale_base:
  scale_n = this->layer_param_.softmax_t_n_param().scale();
  base_n = this->layer_param_.softmax_t_n_param().base();
}
...
template <typename Dtype>
void DistanceWeightedValidPairLossV12Layer<Dtype>::Forward_process_after_layer_parameters_synchronisation(
  const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top){
  //set the input data
...
void DistanceWeightedValidPairLossV12Layer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[0]) {
    ...
    //synchronise the parameters
    auxiLayer->negative_threshold = negative_threshold;
    auxiLayer->pn_margin = pn_margin;
    //forward process
    Forward_process_after_layer_parameters_synchronisation(bottom, top);
    ...
    for(int i = 0; i < pair_num; i++){
      ...
      if( static_cast<int>(pair_label.cpu_data()[i]) ){
        ...
        //Dtype m_dist = dist - (negative_threshold - pn_margin);
        Dtype m_dist = dist - ( Dtype(1) - pn_margin/Dtype(2) );

        //weight computation
        if( m_dist > Dtype(0.0) ){
          pair_weights[i] = power_x_scale_base(m_dist, scale_p, base_p);
        }
        else{
        ...
        //dissimilar pairs
        //Dtype m_dist = negative_threshold - dist;
        Dtype m_dist = ( Dtype(1) + pn_margin/Dtype(2) ) - dist;
        if( m_dist > Dtype(0.0) ){
    ...
    for(int i = 0; i < pair_num; i++){
      ...
      if( static_cast<int>(pair_label.cpu_data()[i]) ){
        //similar pairs
        weight_b = 0.5 * pair_weights[i] / sum_sample_weights_sim[pair_index_b[i]]; //Note that the order 
        weight_a = 0.5 * pair_weights[i] / sum_sample_weights_sim[pair_index_a[i]]; //Note that the order
      }
      else{
        //dissimilar pairs
        weight_b = 0.5 * pair_weights[i] / sum_sample_weights_dissim[pair_index_b[i]]; //Note that the order 
        weight_a = 0.5 * pair_weights[i] / sum_sample_weights_dissim[pair_index_a[i]]; //Note that the order 
      }
      //Note that the order
