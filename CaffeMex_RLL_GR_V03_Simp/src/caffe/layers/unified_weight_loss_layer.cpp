...
#include "caffe/layers/unified_weight_loss_layer.hpp"
#include "caffe/util/math_functions.hpp"

namespace caffe {

template <typename Dtype>
void UnifiedWeightLossLayer<Dtype>::LayerSetUp(
...
template <typename Dtype>
void UnifiedWeightLossLayer<Dtype>::Reshape(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
Dtype UnifiedWeightLossLayer<Dtype>::get_normalizer(
...
template <typename Dtype>
void UnifiedWeightLossLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  ...
  for (int i = 0; i < outer_num_; ++i) {
    for (int j = 0; j < inner_num_; j++) {
      ...
      //changes
      Dtype temp = softmaxT(lamda_p, Dtype(1), 
...
template <typename Dtype>
void UnifiedWeightLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
STUB_GPU(UnifiedWeightLossLayer);
#endif

INSTANTIATE_CLASS(UnifiedWeightLossLayer);
REGISTER_LAYER_CLASS(UnifiedWeightLoss);
