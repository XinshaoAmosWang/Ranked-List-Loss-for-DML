function [batch_data, batch_label] = get_train_minibatch_Mars_CK_X6 (tracklet_num, frame_per_tracklet, ...
                                        batch_data, batch_label, batch_size, ...
                                        tracklets, labels, ...
                                        crop_padding, force_square_size,  cropped_size)
    % sample severals ids
    ids = unique(labels);
    selects = randperm(length(ids), tracklet_num/6);
    selects = [selects selects selects selects selects selects];
